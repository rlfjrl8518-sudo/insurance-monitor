 ...
 * 소재유형/보종/소구포인트/후킹방식 분류를 수정하거나 "설정" 시트(광고주/분류 카테고리 목록)를 관리한다.
 *
 * [배포 방법]
...
var 고정_분류_컬럼 = ["소재유형", "보종", "소구포인트", "후킹방식", "자사"];
...
/** "설정" 시트에서 카테고리별 광고주/소재유형/보종/소구포인트/후킹방식 목록을 읽어온다.
 *
 * "소재유형"/"보종"/"소구포인트"/"후킹방식"을 제외한 모든 컬럼은 광고주 카테고리로 취급하며,
 * 결과의 카테고리 항목에 { 카테고리명: [광고주, ...] } 형태로 담긴다.
 */
function 설정값_가져오기() {
  var 결과 = { 카테고리: {}, 소재유형: [], 보종: [], 소구포인트: [], 후킹방식: [], 자사: [], 광고주그룹: {} };
...
/** ad_id로 행을 찾아 소재유형/보종/소구포인트/후킹방식 컬럼을 수정한다. */
function 광고_수정(ad_id, 소재유형, 보종, 소구포인트, 후킹방식) {
  var sheet = 시트_가져오기(시트이름_데이터);
  ...
  for (var i = 1; i < 값.length; i++) {
    if (String(값[i][ad_id_열]) === String(ad_id)) {
      ...
      if (후킹방식_열 >= 0) sheet.getRange(i + 1, 후킹방식_열 + 1).setValue(후킹방식);
      return { success: true };
    }
  }
...
/** 설정 화면에서 입력한 카테고리별 광고주/소재유형/보종/소구포인트/후킹방식 목록으로 "설정" 시트를 다시 작성한다.
 *
