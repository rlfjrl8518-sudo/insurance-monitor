...
- 기존 광고: 수집일/상태/운영일수/종료일 등 자동 갱신 컬럼만 업데이트
  (소재유형/보종/소구포인트/후킹방식/요약은 대시보드에서 수동 편집하는 값이므로 덮어쓰지 않음)
...
시트_컬럼 = [
    ...
    "소재유형", "보종", "소구포인트", "후킹방식", "요약", "광고텍스트",
    "광고시작일", "광고종료일", "광고상세URL", "상태", "운영일수",
]

# 더 이상 쓰이지 않는 예전 컬럼 - 워크시트_가져오기에서 발견되면 삭제한다
폐지된_컬럼 = ["후킹"]
...
def _설정_시트_컬럼_마이그레이션(워크시트, 설정):
    """이미 존재하는 '설정' 시트에서 폐지된_컬럼(예: 후킹)을 삭제하고,
    고정_분류_컬럼 중 아직 없는 것(예: 소구포인트/후킹방식)을 config.json 기본값으로 추가한다.
    """
    기존_헤더 = 워크시트.row_values(1)
    if not 기존_헤더:
        return
    ...
    for 열번호 in 폐지_열번호_목록:
        워크시트.delete_columns(열번호)

    기존_헤더 = 워크시트.row_values(1)
    누락_고정_컬럼 = [컬럼 for 컬럼 in 고정_분류_컬럼 if 컬럼 not in 기존_헤더]
    if not 누락_고정_컬럼:
        return

    시작_열 = len(기존_헤더) + 1
    워크시트.update([기존_헤더 + 누락_고정_컬럼], "A1")

    기본값_목록들 = [설정["classification"].get(컬럼명, []) for 컬럼명 in 누락_고정_컬럼]
    최대길이 = max((len(목록) for 목록 in 기본값_목록들), default=0)
    if 최대길이:
        데이터 = [
            [목록[i] if i < len(목록) else "" for 목록 in 기본값_목록들]
            for i in range(최대길이)
        ]
        워크시트.update(데이터, gspread.utils.rowcol_to_a1(2, 시작_열))
...
def 설정_시트_초기화(gc, 설정):
    ...
    이미 존재하면 대시보드에서 관리하는 값을 건드리지 않고 그대로 두되, 폐지된_컬럼(예: 후킹)은
    삭제하고 새로 추가된 고정_분류_컬럼(예: 소구포인트/후킹방식)이 아직 없으면 config.json
    기본값으로 채워서 추가한다 (분류축이 바뀔 때의 마이그레이션).
    """
    스프레드시트 = gc.open_by_key(설정["google_sheets"]["spreadsheet_id"])
...
def 설정_시트_읽기(gc, 설정):
    """'설정' 워크시트에서 카테고리별 광고주/소재유형/보종/소구포인트/후킹방식/자사 목록을 읽어온다.

    고정_분류_컬럼(소재유형/보종/소구포인트/후킹방식/자사)을 제외한 모든 컬럼은 광고주
    카테고리로 취급하며, 결과의 "카테고리" 항목에 {카테고리명: [광고주, ...]} 형태로 담긴다.
    "자사"는 대시보드에서 자사로 선택한 광고주명/그룹명 목록이며, 수집/분류 로직에는
    사용하지 않고 카테고리로 잘못 분류되지 않도록 하기 위해 읽어둔다.

    시트가 없거나 비어 있으면 모든 항목이 빈 값인 딕셔너리를 반환한다.
    """
    결과 = {"카테고리": {}, "소재유형": [], "보종": [], "소구포인트": [], "후킹방식": [], "자사": []}
...
def 설정_동적_적용(설정, 서비스계정_경로):
    """'설정' 시트에 입력된 광고주 카테고리/소재유형/소구포인트/후킹방식 목록이 있으면 설정값을 덮어쓴다.
    ...
    if 시트설정["후킹방식"]:
        설정["classification"]["후킹방식"] = 시트설정["후킹방식"]
...
def 시트_동기화(설정, csv_경로, 이미지_폴더, 서비스계정_경로, 강제_분류_덮어쓰기=False):
    ...
    - 분류_컬럼(소재유형/보종/소구포인트/후킹방식/요약)은 기본적으로 시트 셀이 비어 있을
      때만 채워서 대시보드에서 수동 편집한 값을 보존하지만, 강제_분류_덮어쓰기=True이면
      CSV에 값이 있는 한 무조건 덮어쓴다 (분류 기준 자체를 바꿔 전체를 재작성할 때 사용).
    ...
    for ad_id, 행 in 전체_데이터.items():
        ...
        if ad_id in 기존_위치:
            ...
            for 컬럼명 in 분류_컬럼:
                ...
                새값 = 행.get(컬럼명, "")
                if 새값 and (강제_분류_덮어쓰기 or not 현재값):
                    셀주소 = gspread.utils.rowcol_to_a1(행번호, 열번호 + 1)
