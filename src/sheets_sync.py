...
- 기존 광고: 수집일/상태/운영일수/종료일 등 자동 갱신 컬럼만 업데이트
  (소재유형/보종/소구포인트/요약은 대시보드에서 수동 편집하는 값이므로 덮어쓰지 않음)
...
def 워크시트_가져오기(gc, 설정):
    ...
    if not 기존_헤더:
        ...
        # 새로 추가된 컬럼(예: 소구포인트)은 끝이 아니라 시트_컬럼 순서상
...
def _설정_시트_컬럼_마이그레이션(워크시트, 설정):
    """이미 존재하는 '설정' 시트에서 폐지된_컬럼(예: 후킹, 후킹방식)을 삭제하고,
    고정_분류_컬럼 중 아직 없는 것(예: 소구포인트)을 config.json 기본값으로 추가한다.
    """
...
def 설정_시트_초기화(gc, 설정):
    ...
    이미 존재하면 대시보드에서 관리하는 값을 건드리지 않고 그대로 두되, 폐지된_컬럼(예: 후킹,
    후킹방식)은 삭제하고 새로 추가된 고정_분류_컬럼(예: 소구포인트)이 아직 없으면 config.json
...
def 설정_시트_읽기(gc, 설정):
    """'설정' 워크시트에서 카테고리별 광고주/소재유형/보종/소구포인트/자사 목록을 읽어온다.

    고정_분류_컬럼(소재유형/보종/소구포인트/자사)을 제외한 모든 컬럼은 광고주
...
def 시트_동기화(설정, csv_경로, 이미지_폴더, 서비스계정_경로, 강제_분류_덮어쓰기=False):
    ...
    - 분류_컬럼(소재유형/보종/소구포인트/요약)은 기본적으로 시트 셀이 비어 있을
